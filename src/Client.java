...
public class Client {
    ...
    public static void main(String[] args) {
        try {
            ...
            for(InputTask task:input){
                ...
                ServerInterface server = lbstub.fetchServer(task.zoneId);
                ...
                if(task.methodName =="getTimesPlayedByUser"){
                    sResponse = server.getTimesPlayedByUser(task.argument1, task.argument2);
                }else if(task.methodName == "getTopThreeMusicByUser"){
                    sResponse = server.getTopThreeMusicByUser(task.argument1);
                }else if(task.methodName == "getTopArtistsByUserGenre"){
                    sResponse = server.getTopArtistsByUserGenre(task.argument1, task.argument2);
                }else if(task.methodName == "getTimesPlayed"){
    ...
    private static InputTask createInputTask(String data){
        ...
        if(zoneId == "Zone:1"){
            inputTask.zoneId = 1;
        } else if(zoneId == "Zone:2"){
            inputTask.zoneId = 2;
        } else if(zoneId == "Zone:3") {
            inputTask.zoneId = 3;
        } else if(zoneId == "Zone:4"){
            inputTask.zoneId = 4;
        } else if(zoneId == "Zone:5"){
            inputTask.zoneId = 5;
        }
